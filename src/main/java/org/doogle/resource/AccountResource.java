...
import static io.quarkus.mongodb.panache.common.reactive.Panache.withTransaction;
...
public class AccountResource {
  ...
  public Uni<List<AccountModel>> fetchAccounts() {
    ...
    return AccountEntity.getAll().log("ALL_TICKETS").map(b -> accountMapper.fromAccountEntities(b));
  }
  ...
  public Uni<AccountModel> createUpdateAccount(AccountModel account) {
    ...
    return Uni.createFrom().item(account).map(t -> accountMapper.toAccountEntity(t))
  ...
  public Uni<AccountModel> deleteAccount(String id) {
    ...
    return Uni.createFrom().item(id).map(ObjectIdUtils::toObjectId)
  ...
  public Uni<List<AccountModel>> accountTransfer(String fromAccountIdentifier,
    ...
    return withTransaction(() -> {
      Uni<AccountEntity> sourceAccount = AccountEntity.getCollection()
          .findOneAndUpdate(new Document("accountIdentifier", fromAccountIdentifier),
              inc("accountBalance", amountToTransfer * (-1)),
              new FindOneAndUpdateOptions().returnDocument(ReturnDocument.AFTER).upsert(true))
          .log("SOURCE_ACCOUNT_AFTER");

      Uni<AccountEntity> targetAccount = AccountEntity.getCollection()
          .findOneAndUpdate(new Document("accountIdentifier", toAccountIdentifier),
